...
@compute
@workgroup_size(16, 16, 1)
fn main_sad_mask(@builtin(global_invocation_id) global_id: vec3<u32>) {
    var x = global_id.x;
    var y = global_id.y;
    ...
    for (var i = 0u; i < match_width; i++) {
        for (var j = 0u; j < match_height; j++) {
            var input_idx = (y + j) * input_width + (i + x);
            var template_idx = j * template_width + i;
            var mask_val = mask_buf[template_idx];
...
@compute
@workgroup_size(16, 16, 1)
fn main_ssd_mask(@builtin(global_invocation_id) global_id: vec3<u32>) {
    var x = global_id.x;
    var y = global_id.y;
